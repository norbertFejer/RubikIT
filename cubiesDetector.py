...
class ImageHandler:
    ...
    def __get_four_corners (self, contours, max_accepted_area):
        four_corners = []
    ...
    def get_contours (self, image):
        ...
        edges = self.__get_edges_from_image (original_hsv, 320, 20)
        dilation = self.__dilatate_edges (edges)

        raw_contours = self.__get_contours_from_image (dilation)
        original_img_area = image.shape[0] * image.shape[1]
        max_accepted_area = original_img_area / 3
        four_corners, four_corners_area_sum =  self.__get_four_corners (raw_contours, max_accepted_area)

        avg_area_of_four_corners = four_corners_area_sum / len(four_corners)
        extracted_edges = self.__get_cube_pieces (four_corners, avg_area_of_four_corners)

        sorted_contours = self.__get_sorted_contours (extracted_edges)
